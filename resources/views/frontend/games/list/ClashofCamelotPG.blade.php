...
<base href="/games/{{ $game->name }}/Casino/">
    ...
    <script src="/games/{{ $game->name }}/Content/javascript/platformbundle.html?v=2i2hOXGcd-XCxxc3UDAd23RqqeXLE_dlVDx-sYlmqqE1"></script>
...
<script>
	PngPreloader =
	{
		...
		_showAndHideNyxOgsLoaderMessage: function () { },
		...
		init: function ()
		{
			...
			this.nyxOgsLoaderMessage = document.getElementById("nyxOgsLoaderMessage");
			var cssPath = "/Content/css/preloader/preloader?v=mwkf9h8L--BP7RhC3gPsOK0KwIHwP4bEalzVqW27Vm01";
			this.loadCss(cssPath);

			this.initImplementation();
			this.setupPreventDefaultEvents();
			this._showAndHideNyxOgsLoaderMessage = this.showAndHideNyxOgsLoaderMessage.bind(this);
		},
		...
		showLoaderMessage: function ()
		{
			...
			if (this.nyxOgsLoaderMessage && this.loaderMessageText)
			{
				this.nyxOgsLoaderMessage.textContent = this.loaderMessageText;
				Engage.addEventListener("gameReady", this._showAndHideNyxOgsLoaderMessage);
			}
		...
		showAndHideNyxOgsLoaderMessage: function () {
			const startMessageDiv = document.getElementById("nyxOgsLoaderMessage");

			startMessageDiv.classList.remove("pngRemove");
			startMessageDiv.classList.add("pngShow");

			setTimeout(() => {
				startMessageDiv.classList.remove("pngShow")
				startMessageDiv.classList.add("pngRemove");
			}, 5000);

			Engage.removeEventListener("gameReady", this._showAndHideNyxOgsLoaderMessage);
		},
		...
		loadCss: function (path)
		{
			...
			cssfileref.setAttribute("href", "https://acccw.playngonetwork.com/" + path);
...
<script>
    PngPreloader.initImplementation = function ()
    {
        ...
        var cssPath = "/Content/css/preloader/pngpreloader?v=2cWCqosHzwc1eYMTgN7Kyzk_7eE7RyspMRXRnys7Ju81";
    ...
    <script type="text/javascript">
		...
		GameLauncher = {
			loadGame: function ()
			{
				...
				var src = "../Casino/GameLoader.html?div=pngCasinoGame&pid=530&gid=clashofcamelot&lang=en_US&practice=1&demo=2&isbally=False&fullscreenmode=False&rccurrentsessiontime=0&rcintervaltime=0&autoplaylimits=0&autoplayreset=0&channel=desktop&resourcelevel=0&hasJackpots=False&defaultsound=False&embedmode=iframe&origin=&showPoweredBy=True&country=CW&EnableBundledLaunchers=PlaynGO.CoreWeb.Code.Models.EnabledHeraLaunchers";
				var script = document.createElement('script');
				script.setAttribute('src', src);
				script.addEventListener("load", function ()
				{
					EngageCom.init();
					if ("desktop" === "desktop" || "desktop" === "cabinet")
					{
						Engage.addEventListener("gameHelpOpened", GameLoader.showEmbeddedHelp.bind(GameLoader));
						Engage.addEventListener("gameHistoryOpened", GameLoader.showEmbeddedHistory.bind(GameLoader));
					}


					if ("desktop" === "mini")
					{
						Engage.addEventListener("gameHelpOpened", function (e)
						{

							GameLauncher.openPopup(GameLoader.getHelpUrl(), 360, 640);
						});

						Engage.addEventListener("gameHistoryOpened", function (e)
						{
							GameLauncher.openPopup(GameLoader.getHistoryUrl(), 360, 640);
						});
		...
		EngageCom = {
			...
			origin: null,
			init: function() {
				this.origin = decodeURIComponent("");
			},
			onWrapperMessage: function(e) {
				console.log("Wrapper says: ", e);
				switch (e.data.messageType)
				{
					case "request":
						Engage.request({ req: e.data.request, data: e.data.data });
						break;
					case "addEventListener":
						Engage.addEventListener(e.data.eventType, this.onGameMessage.bind(this));
						break;
				}
			},
			onGameMessage: function(e) {
				console.log("Game says: ", e);
				this.source.postMessage(e, this.origin);
			}
		}
		window.addEventListener('message', EngageCom.onWrapperMessage.bind(EngageCom));
    </script>
