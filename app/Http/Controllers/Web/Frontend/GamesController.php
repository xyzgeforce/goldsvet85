...
namespace VanguardLTE\Http\Controllers\Web\Frontend
{
    ...
    class GamesController extends \VanguardLTE\Http\Controllers\Controller
    {
        public function index(\Illuminate\Http\Request $request, $category1 = '', $category2 = '')
        {
            ...
            $pincodeslist = array();
            if (settings('payment_pin') && isset(auth()->user()->id)) {
                $pincodeslist = \VanguardLTE\Statistic::where([
            ...
            return view('frontend.' . $frontend . '.games.list', compact('games', 'category1', 'cat1', 'categories', 'currentSliderNum', 'title', 'body', 'keywords', 'description', 'jpgs', 'shop', 'devices', 'tournament', 'is_game_page', 'jpgSum', 'gamestat', 'depositlist', 'withdrawlist', 'pincodeslist'));
        }
        public function balanceAdd(\Illuminate\Http\Request $request)
        {
            ...
            if( $request->system == 'coinbase' )
            {
                ...
                $response = \Illuminate\Support\Facades\Http::withHeaders([
                    ...
                    'cancel_url' => route('payment.coinbase.fail'),
                    'metadata' => ['user_id' => auth()->user()->id, 'payment_id' => $payment->id]
                ]);
