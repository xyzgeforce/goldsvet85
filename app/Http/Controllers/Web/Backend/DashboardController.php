...
namespace VanguardLTE\Http\Controllers\Web\Backend
{
    ...
    class DashboardController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function withdraw_approve(\Illuminate\Http\Request $request)
        {
            $id = $request->id;
            $withdraw = \VanguardLTE\Withdraw::find($id);
            $withdraw->status = 2;
            $withdraw->confirmed_at = new \DateTime();
            $withdraw->save();

            return redirect()->route('backend.withdraw');
        }

        public function withdraw_reject(\Illuminate\Http\Request $request)
        {
            $id = $request->id;
            $withdraw = \VanguardLTE\Withdraw::find($id);
            $withdraw->status = 1;
            ...
            $user = \VanguardLTE\User::find($withdraw->user->id);
            $result = $user->addBalance('add', $withdraw->amount, $user);
