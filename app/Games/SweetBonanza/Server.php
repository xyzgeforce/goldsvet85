...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                ///////////////////////////////////////////
					...
					for( $jp = 0; $jp < count($jpgs); $jp++ ) 
					{
						if( $jpgs[$jp]->balance != '' && $jpgs[$jp]->balance < $jpgs[$jp]->get_min('start_balance') ) 
						{
							...
							if( $summ > 0 ) 
							{
								$ekliyormu = $jpgs[$jp]->add_jpg('add', $summ);
								\Illuminate\Support\Facades\Log::info('testing');
								// \Illuminate\Support\Facades\Log::info(json_encode($ekliyormu));
							}
						}
					}
					//final
				...
				/* if(isset($jpgs) && $jpgs && $request->input('action') && $request->input('action') == 'doSpin'){
					...
					for( $i = 0; $i < count($jpgs); $i++ ) 
					{
						if( $count_balance == 0 ) 
						{
							file_put_contents(storage_path('logs/') . '8feb_test.txt','step1'.PHP_EOL, FILE_APPEND);
							$jsum[$i] = $jpgs[$i]->balance;
						}
						else if( $count_balance < $bet ) 
						{
							file_put_contents(storage_path('logs/') . '8feb_test.txt','step2'.PHP_EOL, FILE_APPEND);
							$jsum[$i] = $count_balance / 100 * $jpgs[$i]->percent + $jpgs[$i]->balance;
						}
						else
						{
							file_put_contents(storage_path('logs/') . '8feb_test.txt','step3'.PHP_EOL, FILE_APPEND);
							$jsum[$i] = $bet / 100 * $jpgs[$i]->percent + $jpgs[$i]->balance;
						}
						if( $jpgs[$i]->get_pay_sum() < $jsum[$i] && $jpgs[$i]->get_pay_sum() > 0 ) 
						{
							file_put_contents(storage_path('logs/') . '8feb_test.txt','step4'.PHP_EOL, FILE_APPEND);
							if( $jpgs[$i]->user_id && $jpgs[$i]->user_id != $user->id ) 
							{
								
								file_put_contents(storage_path('logs/') . '8feb_test.txt','step5'.PHP_EOL, FILE_APPEND);
							}
							else
							{
								file_put_contents(storage_path('logs/') . '8feb_test.txt','step6'.PHP_EOL, FILE_APPEND);
						...
						file_put_contents(storage_path('logs/') . '8feb_test.txt','Jsum i: balance e: ' .$jsum[$i] .PHP_EOL, FILE_APPEND);
						$jpgs[$i]->balance = $jsum[$i] + (($bet * $lines) / 100);
						if( isset($payJack) &&  $payJack > 0 )$jpgs[$i]->user_id = NULL;
						if( isset($isJackPay) && $isJackPay === 1 )$jpgs[$jp - 1]->user_id = NULL;
						$jpgs[$i]->save();
						file_put_contents(storage_path('logs/') . '8feb_test.txt','saved'.PHP_EOL, FILE_APPEND);
						if( $jpgs[$i]->balance < $jpgs[$i]->get_min("start_balance") ) 
						{
							$summ = $jpgs[$i]->get_start_balance();
							file_put_contents(storage_path('logs/') . '8feb_test.txt','step8 summ:' . $summ .PHP_EOL, FILE_APPEND);
							if( $summ > 0 ) 
							{
								
								file_put_contents(storage_path('logs/') . '8feb_test.txt','step9'.PHP_EOL, FILE_APPEND);
