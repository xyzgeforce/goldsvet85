...
class SlotArea
{
    ...
    public static function handleRS($gameSettings, &$slotArea, $log, $win){
        if($log && array_key_exists('rs_c', $log)){
            if($log['rs_c'] == $log['rs_m'])
                $slotArea['rs_t'] = $log['rs_m'];
            else {
                $slotArea['rs_c'] = $log['rs_c'] + 1;
                $slotArea['rs_p'] = $log['rs_p'] + 1;
                $slotArea['rs_m'] = $log['rs_m'];
            }
            if($log['rs_c'] == 1)
                $slotArea['rs_win'] = 0;
            else $slotArea['rs_win'] = $log['rs_win'];
            $slotArea['rs_win'] += $win['TotalWin'];
        }
    }
}
