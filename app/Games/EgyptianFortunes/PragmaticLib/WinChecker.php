...
class WinChecker
{
    ...
    public function __construct($gameSettings){
        ...
        $rs_cfg = explode(';', $gameSettings['rs_cfg']);
        ...
        $this->rs_cfg = [];
        foreach($rs_cfg as $rs){
          $rs = explode('~', $rs);
          $this->rs_cfg[$rs[1]] = ['sym_cnt' => $rs[2], 'rs_cnt' => $rs[3]];
        }
    }
    ...
    public function getWin($pur, $log, $bet,&$SlotArea){
        ...
        foreach($this->paylines as $index => $payline){
            ...
            if ($win > 0){
                ...
                // check if there is a trigger of the RS
                if($this->rs_cfg[$winSymbol]['sym_cnt'] <= $cnt && !array_key_exists('rs_c', $log)){
                  if(array_key_exists('rs_m', $SlotArea) && $SlotArea['rs_m'] < $this->rs_cfg[$winSymbol]['rs_cnt']
                  || !array_key_exists('rs_m', $SlotArea)){
                    $SlotArea['rs_m'] = $this->rs_cfg[$winSymbol]['rs_cnt'];
                    $SlotArea['rs_c'] = 1;
                    $SlotArea['rs_p'] = 0;
                    $SlotArea['rs'] = 'l~'.$winSymbol.';'.$index.';'.implode(',', $winPositions);
                  }
                }
