...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
		...
		if(!isset($win['rswin'])) $win['rswin'] = 0;
		if(($win["TotalWin"] + $fswin)>0){
			$checkUserRtp = \VanguardLTE\Games\PragmaticLib\CheckUserRtp::user_permission($user, $gameSettings, $bet * $lines, $win["TotalWin"] + $win['rswin'], $fswin + $win['rswin']);
