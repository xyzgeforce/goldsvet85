...
class SlotArea
{
    ...
    public static function setWildGroup(&$slotArea, $log){
        $limit = 10;
        if($log && array_key_exists('fs', $log))
            $limit = 50;
        // Make 2X2 wilds area
        if(rand(0, 100) < $limit){
            var_dump('slotarea = ', $slotArea['SlotArea']);
            // Remove all the wilds in the slot area
            $wildPositions = array_keys($slotArea['SlotArea'], 2);
            foreach($wildPositions as $pos)
                $slotArea['SlotArea'][$pos] = ''.rand(3, 11);
            // Now we make 2X2 wild area
            $startPos = [0,1,2,3,5,6,7,8,10,11,12,13];  //  start position of the 2X2 area
            $startPos = $startPos[rand(0, 11)]; // select the start position randomly among the possible poses
            $offsets = [0, 1, 5, 6];    // offsets from start position
            $slotArea['is'] = $slotArea['SlotArea'];    // set the original slot area
            $slotArea['rwd'] = [];  //  set of wild positions "2~18,19,23,24"
            foreach($offsets as $offset){
                $wildPos = $startPos + $offset;
                var_dump('wildPos='.$wildPos);
                $slotArea['SlotArea'][$wildPos] = 2;
                $slotArea['rwd'][] = $wildPos;
            }
            $slotArea['rwd'] = '2~'.implode(',', $slotArea['rwd']);
            // set random multiplier 2X, 3X, 5X, 10X, 25X
            if($log && array_key_exists('fs', $log)){
                $wmv = [2, 3, 5, 10, 25];
                $ind = 0;
                $rand = rand(0, 100);
