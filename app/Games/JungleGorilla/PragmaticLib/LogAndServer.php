...
class LogAndServer
{
    public static function getResult($slotArea, $index, $counter, $bet, $lines, $reelSet, $win, $pur, 
        ...
        $toServer = [
            ...
            'sb='.implode(',', $toLog['sb']),
            'accm=cp~np~mp;cp~np~mp;cp~np~mp;cp~np~mp;cp~np~mp',
            'acci='.implode(';', $toLog['acci']),
            'mbri='.implode(';', $toLog['mbri']),
            'sh='.$gameSettings['sh'],
            'c='.$toLog['Bet'],
            'sver=5',
        ...
        $addKeys = ['is', 'accv', 'rwd', 'mbr', 'mbv', 'mbp'];
        foreach($addKeys as $val){
            var_dump($val);
            if(array_key_exists($val, $slotArea)){
                $toLog[$val] = $slotArea[$val];
                if(!is_array($toLog[$val]))
                    $toServer[] = $val.'='.$toLog[$val];
                else if($val == 'accv'){
                    foreach($slotArea[$val] as $key => $accv){
                        $slotArea[$val][$key] = implode('~', $accv);
                    }
                    $toServer[] = $val.'='.implode(';', $slotArea[$val]);
                }
                else $toServer[] = $val.'='.implode(',', $toLog[$val]);
