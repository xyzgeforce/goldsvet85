    ...
    public function getWin($pur, $log, $bet, &$SlotArea){
        ...
        foreach($this->paylines as $index => $payline){
            ...
            if ($win > 0){
				...
				var_dump('1_3');
        var_dump('winWildLine='.implode(',', $winWildLine));
        foreach($winWildLine as $lineKey => $lineIdx){
          foreach($this->paylines[$lineIdx] as $key => $pos){
            if($SlotArea['SlotArea'][$pos * 5 + $key] == 2 && $multis[$lineKey] > 1){
              $SlotArea['accv'][$key][1] = 1;
              $multis[$lineKey] /= $SlotArea['mbr'][$key];
            }
          }
        }
