...
namespace VanguardLTE\Games\JungleGorilla\PragmaticLib;
...
class SlotArea
{
    ...
    public static function setAccv(&$slotArea, $log){
        $isAccv = 0;
        $rwd = '';
        $mbr = [];
        if($log)
            $accv = $log['accv'];
        else $accv = [[1, 1, 5], [1, 1, 5], [1, 1, 5], [1, 1, 5], [1, 1, 5]];
        foreach($accv as $key => $val){
            $mbr[] = $val[1];
            $accv[$key][0] = $val[1];
        }
        $is = $slotArea['SlotArea'];
        var_dump('accv=', $accv);
        var_dump('mbr='.implode(',', $mbr));
        foreach($slotArea['SlotArea'] as $key => $value){
            if($value == 1){
                var_dump('accv_0');
                $isAccv = 1;
                $slotArea['SlotArea'][$key] = 12;
                var_dump('accv_1');
                $rwd = '12~'.$key;
                $reel = $key % 5;
                $addMulti = self::getMulti();
                var_dump('accv_2');
                $mbr[$reel] = $mbr[$reel] + $addMulti > 5 ? 5 : $mbr[$reel] + $addMulti;
                var_dump('accv_3');
                $accv[$reel][0] = $mbr[$reel];
                $accv[$reel][1] = $mbr[$reel];
                var_dump('rwd='.$rwd);
                var_dump('accv=', $accv);
                var_dump('mbr='.implode(',', $mbr));
                break;
        ...
        if(array_key_exists(2, array_count_values($slotArea['SlotArea']))){
            $mbp = [];
            $mbv = [];
            foreach($slotArea['SlotArea'] as $key => $val){
                if($val == 2){
                    var_dump('!!!1');
                    $reel = $key % 5;
                    var_dump('!!!2');
                    ...
                    $mbv[] = $mbr[$reel];
                    var_dump('!!!4');
                    var_dump('mbp='.implode(',', $mbp));
                    var_dump('mbv='.implode(',', $mbv));
                }
            }
            $slotArea['mbp'] = $mbp;
            $slotArea['mbv'] = $mbv;
    ...
    private static function getMulti(){
        return 1;
    }
}
