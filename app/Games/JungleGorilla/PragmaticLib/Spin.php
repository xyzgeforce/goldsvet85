...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
       ...
       // if ($user->balance < $bet * $lines) return false;
        ...
        var_dump('1');
        if(!$currentLog || $currentLog && array_count_values($currentLog['mbr'])[1] > 2)
        BuyFreeSpins::addScatters($slotArea['SlotArea'], $gameSettings, 0, 1); // add scatters
        // Set Accv
        SlotArea::setAccv($slotArea, $currentLog);
